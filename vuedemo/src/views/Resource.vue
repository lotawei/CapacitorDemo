...
import { brCall, getBRData } from '../composables/capacitor-bridge'
...
async function refreshStatus() {
  try {
    status.value = await brCall('resource.getStatus') as any || status.value
  } catch (e: any) {
    addLog(`❌ ${e.message || e}`)
  }
}
...
async function checkUpdate() {
  ...
  addLog('🔍 检查更新...')
  try {
    const r = await brCall('resource.checkUpdate') as any
    if (r.hasUpdate) {
...
async function startUpdate() {
  ...
  addLog('📥 开始下载...')
  try {
    const r = await brCall('resource.startUpdate') as any
    if (r.ok) {
