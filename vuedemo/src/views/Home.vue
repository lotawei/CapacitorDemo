...
import { useBridge, getBRData } from '../composables/capacitor-bridge'
import { setBridgeMeta } from '../composables/capacitor-bridge'
...
async function addToGallery(item: any) {
  ...
  if (gi.type === 'image' && gi.path) {
    try {
      ...
      gi.dataUrl = (data as any)?.data?.dataUrl ?? (data as any)?.dataUrl as string | undefined
